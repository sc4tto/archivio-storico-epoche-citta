...
def knowledge(data,sts):
    ...
    root=nodes[0];s=stmap(sts,root['status']);p.append(f'<g class="node" data-id="{esc(root["id"])}" data-status="{esc(root["status"])}"><circle cx="{cx}" cy="{cy}" r="82" fill="#2e493f" stroke="{s["color"]}" stroke-width="1.45"/>{textlines(cx,cy,root["label"],22,16,700,"#fff",3)}</g>')
    for c in cats:
        x,y=pos[c['id']];s=stmap(sts,c['status']);p.append(f'<g class="node" data-id="{esc(c["id"])}" data-status="{esc(c["status"])}"><title>{esc(c["label"])} — {esc(s["label"])}</title><circle cx="{x:.1f}" cy="{y:.1f}" r="55" fill="#f2eadc" stroke="{s["color"]}" stroke-width="2.1"/>{textlines(x,y,c["label"],19,12,700,"#1f2933",3)}</g>')
    for c,ch in leaves:
        x,y=pos[ch['id']];s=stmap(sts,ch['status']);anchor='start' if x>=cx else 'end';dx=14 if anchor=='start' else -14
        p.append(f'<g class="node" data-id="{esc(ch["id"])}" data-status="{esc(ch["status"])}"><title>{esc(ch["label"])} — {esc(s["label"])}</title><circle cx="{x:.1f}" cy="{y:.1f}" r="9" fill="#fff" stroke="{s["color"]}" stroke-width="2.1"/><text x="{x+dx:.1f}" y="{y:.1f}" text-anchor="{anchor}" dominant-baseline="middle" font-size="12" font-weight="600" fill="#1f2933">{esc(ch["label"])}</text></g>')
    p.append('</svg>');return ''.join(p)

...
FILES={"master":"gerico_master.json","food":"01_alimentazione.json","places":"02_provenienze.json","settlement":"03_insediamento.json","building":"04_costruzione.json","knowledge":"05_conoscenze.json","claims":"06_fonti_certezza.json"}
...
def wrap(text,maxc=27,maxl=3):
    ...
    return lines or ['']
def textlines(x,y,text,maxc=27,fs=12,weight=600,fill='#1f2933',maxl=3):
...
def nav(cur):
    items=[('../index.html','Indice','index')]+[(f,l,f) for f,l,_,_ in PAGES]
...
def layered(data,sts,titles,height):
    ...
    for i,t in enumerate(titles):
        if i in xs:p.append(f'<text x="{xs[i]:.1f}" y="42" text-anchor="middle" font-size="16" font-weight="700" fill="#2e493f">{esc(t)}</text>')
    for e in edges:
    ...
    for n in nodes:
        ...
        p.append(f'<g class="node" data-id="{esc(n["id"])}" data-status="{esc(n["status"])}"><title>{esc(n["label"])} — {esc(s["label"])}</title><rect x="{x-nw/2:.1f}" y="{y-nh/2:.1f}" width="{nw}" height="{nh}" rx="12" fill="#fffdf8" stroke="{s["color"]}" stroke-width="2" filter="url(#shadow)"/>{textlines(x,y,n["label"])}</g>')
...
def provenance(data,sts):
    ...
    pos={p['id']:proj(p) for p in local};p=[f'<svg viewBox="0 0 {W} {H}" role="img">',defs(),f'<rect x="{px0}" y="{py0}" width="{pw}" height="{ph}" rx="18" fill="#fbfaf6" stroke="#d6d0c4"/>']
    for i in range(1,5):
        x=px0+i*pw/5;lon=lonmin+i*(lonmax-lonmin)/5;p+= [f'<line x1="{x:.1f}" y1="{py0}" x2="{x:.1f}" y2="{py0+ph}" stroke="#e7e1d7"/>',f'<text x="{x:.1f}" y="{py0+ph+22}" text-anchor="middle" font-size="11" fill="#1f2933">{lon:.2f}°E</text>']
    for i in range(1,4):
        y=py0+i*ph/4;lat=latmax-i*(latmax-latmin)/4;p += [f'<line x1="{px0}" y1="{y:.1f}" x2="{px0+pw}" y2="{y:.1f}" stroke="#e7e1d7"/>',f'<text x="{px0-12}" y="{y:.1f}" text-anchor="end" dominant-baseline="middle" font-size="11" fill="#1f2933">{lat:.2f}°N</text>']
    p += ['<text x="615" y="88" text-anchor="middle" font-size="18" font-weight="700" fill="#2e493f">Territori di approvvigionamento — coordinate moderne indicative</text>','<rect x="1200" y="92" width="270" height="195" rx="16" fill="#f5efe3" stroke="#d6d0c4"/>','<text x="1335" y="116" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2933">Connessioni a lunga distanza</text>']
    dpos={}
    for i,q in enumerate(distant):
        x,y=1335,160+i*74;dpos[q['id']]=(x,y);s=stmap(sts,q['status']);p.append(f'<g class="node" data-id="{esc(q["id"])}" data-status="{esc(q["status"])}"><circle cx="{x}" cy="{y}" r="9" fill="#fff" stroke="{s["color"]}" stroke-width="3"/>{textlines(x,y+31,q["label"],30,11,600,"#1f2933",3)}</g>')
    apos={**pos,**dpos}
    for f in flows:
...
def timeline(data,sts):
    ...
    for i,phase in enumerate(phases):
        x=left+i*pw;fill='#faf7f0' if i%2==0 else '#f4efe4';p += [f'<rect x="{x:.1f}" y="{top}" width="{pw:.1f}" height="{H-top-35}" fill="{fill}" stroke="#ded7ca"/>',f'<text x="{x+pw/2:.1f}" y="76" text-anchor="middle" font-size="18" font-weight="700" fill="#2e493f">{esc(phase)}</text>']
    for i,lane in enumerate(lanes):
        cy=top+(i+.5)*lh;p += [f'<text x="{left-18}" y="{cy:.1f}" text-anchor="end" dominant-baseline="middle" font-size="13" font-weight="700" fill="#4b5563">{esc(lane)}</text>',f'<line x1="{left}" y1="{top+(i+1)*lh:.1f}" x2="{W-right}" y2="{top+(i+1)*lh:.1f}" stroke="#e4ded2"/>']
    for it in items:
...
def knowledge(data,sts):
    W,H=1500,1100;cx,cy=W/2,H/2;cats=data.get('children',[]);pos={data['id']:(cx,cy)};r1,r2=225,425;n=len(cats)
    for i,c in enumerate(cats):
        a=-math.pi/2+i*2*math.pi/n;pos[c['id']]=(cx+r1*math.cos(a),cy+r1*math.sin(a));ch=c.get('children',[]);offs=[0] if len(ch)==1 else [(j-(len(ch)-1)/2)*.14 for j in range(len(ch))]
        for child,o in zip(ch,offs):pos[child['id']]=(cx+r2*math.cos(a+o),cy+r2*math.sin(a+o))
    nodes=[];edges=[]
    def walk(node,parent=None,depth=0):
        z=dict(node);z['_depth']=depth;nodes.append(z)
        if parent:edges.append((parent,node['id']))
        for c in node.get('children',[]):walk(c,node['id'],depth+1)
    walk(data);by={x['id']:x for x in nodes};p=[f'<svg viewBox="0 0 {W} {H}" role="img">',defs()]
    for a,b in edges:
    ...
    for q in nodes:
        x,y=pos[q['id']];dep=q['_depth'];s=stmap(sts,q['status']);w,h,r,fill,tf=(190,84,42,'#2e493f','#fff') if dep==0 else ((168,62,14,'#fffdf8','#1f2933') if dep==1 else (180,58,12,'#fff','#1f2933'))
        p.append(f'<g class="node" data-id="{esc(q["id"])}" data-status="{esc(q["status"])}"><title>{esc(q["label"])} — {esc(s["label"])}</title><rect x="{x-w/2:.1f}" y="{y-h/2:.1f}" width="{w}" height="{h}" rx="{r}" fill="{fill}" stroke="{s["color"]}" stroke-width="2" filter="url(#shadow)"/>{textlines(x,y,q["label"],24,12,600,tf,3)}</g>')
    p.append('</svg>');return ''.join(p)
...
def generate(data_dir:Path,out:Path,root:Path):
    ...
    body=f'<div class="card intro"><div><h2>Modulo pilota generato da JSON</h2><p>Le sei visualizzazioni sono prodotte automaticamente dai file nella cartella <code>dati</code>. Modificando i JSON e avviando <code>RIGENERA_GERICO.bat</code>, le pagine vengono ricostruite.</p><div class="note">{esc(meta.get("warning",""))}</div></div><div><h3>Localizzazione</h3><p><strong>{esc(meta.get("site",""))}</strong><br>{esc(meta.get("modern_location",""))}<br>{esc(meta.get("coordinates",{}).get("lat",""))} N, {esc(meta.get("coordinates",{}).get("lon",""))} E</p></div></div><div class="card"><h2>Documenti</h2><div class="cards">{cards}</div></div><div class="card"><h2>Metadati</h2><div class="meta-grid"><div class="meta-box"><strong>Ambito</strong><br>{esc(meta.get("scope",""))}</div><div class="meta-box"><strong>Versione dati</strong><br>{esc(meta.get("version",""))}</div><div class="meta-box"><strong>Fonti registrate</strong><br>{len(master.get("sources",[]))}</div></div></div><div class="card"><h2>File dati canonici</h2><pre>{esc(chr(10).join(FILES.values()))}</pre></div>'
    index=f'<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(display)} — modulo multi-grafo JSON</title><link rel="stylesheet" href="{cssc}"><script defer src="{jsc}"></script></head><body><header><h1>{esc(display)} — modulo multi-grafo JSON</h1><p>{esc(meta.get("scope",""))}</p></header><nav><a href="index.html" aria-current="page">Indice</a>{"".join(f"<a href=\"documenti/{f}\">{esc(l)}</a>" for f,l,_,_ in PAGES)}</nav><main>{body}</main></body></html>'

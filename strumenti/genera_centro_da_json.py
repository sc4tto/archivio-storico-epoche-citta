...
def layered(data,sts,titles,height):
    ...
    for n in nodes:
        ...
        p.append(f'<g class="node" data-id="{esc(n["id"])}" data-status="{esc(n["status"])}"><title>{esc(n["label"])} — {esc(s["label"])}</title><rect x="{x-nw/2:.1f}" y="{y-nh/2:.1f}" width="{nw}" height="{nh}" rx="4" fill="#fffdf8" stroke="{s["color"]}" stroke-width="1.45"/>{textlines(x,y,n["label"])}</g>')
...
def provenance(data,sts):
    ...
    pos={p['id']:proj(p) for p in local};p=[f'<svg viewBox="0 0 {W} {H}" role="img">',defs(),f'<rect x="{px0}" y="{py0}" width="{pw}" height="{ph}" rx="4" fill="#fbfaf6" stroke="#d6d0c4"/>']
    for i in range(1,5):
    ...
    p += ['<text x="615" y="88" text-anchor="middle" font-size="18" font-weight="700" fill="#2e493f">Territori di approvvigionamento — coordinate moderne indicative</text>','<rect x="1200" y="92" width="270" height="195" rx="4" fill="#f5efe3" stroke="#d6d0c4"/>','<text x="1335" y="116" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2933">Connessioni a lunga distanza</text>']
    dpos={}
    for i,q in enumerate(distant):
        x,y=1335,160+i*74;dpos[q['id']]=(x,y);s=stmap(sts,q['status']);p.append(f'<g class="node" data-id="{esc(q["id"])}" data-status="{esc(q["status"])}"><circle cx="{x}" cy="{y}" r="9" fill="#fff" stroke="{s["color"]}" stroke-width="2.1"/>{textlines(x,y+31,q["label"],30,11,600,"#1f2933",3)}</g>')
    apos={**pos,**dpos}
    for f in flows:
...
def knowledge(data,sts):
    ...
    for q in nodes:
        ...
        p.append(f'<g class="node" data-id="{esc(q["id"])}" data-status="{esc(q["status"])}"><title>{esc(q["label"])} — {esc(s["label"])}</title><rect x="{x-w/2:.1f}" y="{y-h/2:.1f}" width="{w}" height="{h}" rx="{r}" fill="{fill}" stroke="{s["color"]}" stroke-width="1.45"/>{textlines(x,y,q["label"],24,12,600,tf,3)}</g>')
    p.append('</svg>');return ''.join(p)
